...
df_m <- df_clean[c('user_id', 'order_id', 'problem_id', 'correct', 'hint_count', 'hint_total', 'bottom_hint', 'ms_first_response', 'attempt_count')]
df_hist <- merge(df_m, df_m, by = 'user_id', all.x = TRUE, allow.cartesian=TRUE)
df_hist <- df_hist %>% filter(order_id.y < order_id.x)
df_hist <- df_hist %>% group_by(order_id.x) %>% 
...
#df_fin <-  df_clean %>% left_join(df_hist, by = order_id)
...
```{r}
#fit a logistic regression model on train using 4 features based on forward stepwise selection
...
The AUC of the logistic model predicting hint-taking on the test dataset is 85.17075
...
The AUC of the logistic model predicting hint-taking on the test dataset is 84.97453 (*decreased from 85.2)
...
re_lg_bottom_hint <- glm(bottom_hint ~ prev_problem_requested_hint +
                           prev_percent_problem_requested_hint +
                           prev_total_bottom_hint +
                           prev_percent_bottom_hint,
                          data=train, family="binomial")
...
```
The AUC of the revised logistic model predicting bottom_hint on the test dataset is 81.73045
...
rf_bottom_hint <- randomForest(bottom_hint ~ prev_problem_requested_hint +
                           prev_percent_problem_requested_hint +
                           prev_total_bottom_hint +
                           prev_percent_bottom_hint,
                          data=train, ntree=200, na.action=na.omit)
...
The AUC score of randomforest model for predicting bottom_hint on the test dataset is 64.05265
