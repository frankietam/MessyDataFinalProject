...
df_join <- left_join(Orders, df_m, by=c("user_id", "order_id.y" = "order_id"))
...
the model with the following five features was selected based on the results;
  features included: prev_total_correct, prev_problem_requested_hint, 
    prev_percent_problem_requested_hint, prev_avg_response_time, prev_percent_bottom_hint
  features excluded: prev_percent_correct, prev_total_bottom_hint, prev_avg_attempt_count
...
re_lg_hint <- glm(first_action ~ prev_total_correct +
                 prev_problem_requested_hint + prev_percent_problem_requested_hint +
                 prev_avg_response_time + prev_percent_bottom_hint,
...
```
the auc score is  84.16235 (a random forest model has a better AUC score)
...
the model with the following six features was selected based on the results;
  features included: prev_total_correct, prev_problem_requested_hint, 
    prev_percent_problem_requested_hint, prev_avg_response_time, 
    prev_total_bottom_hint, prev_percent_bottom_hint, 
  features excluded: prev_percent_correct, prev_avg_attempt_count
...
```
The AUC of the revised logistic model predicting bottom_hint on the test dataset is 78.48752
...
the model with the following six features was selected based on the results;
  features included: prev_total_correct, prev_percent_correct, prev_problem_requested_hint,
    prev_percent_problem_requested_hint, prev_avg_response_time, prev_percent_bottom_hint
  features excluded: prev_total_bottom_hint, prev_avg_attempt_count
